  ...
  <UCard class="flex-[50%]">
    ...
    <div class="flex flex-col gap-4">
      <FormGroup :name="$t('config.theme')">
        <USelectMenu
          v-model="configMeta.theme" value-key="id" label-key="label" icon="i-gridicons-themes"
          :items="themeList" :search-input="false" class="w-full"
        />
      </FormGroup>
      ...
      <FormGroup :name="$t('config.bg.name')">
        <USelectMenu
          v-model="configMeta.bgType" value-key="id" label-key="label" icon="i-heroicons-photo"
          :items="bgTypes" :search-input="false" class="w-full"
        />
      </FormGroup>
      ...
      <FormGroup :name="$t('config.icon.name')" :help="$t('config.icon.helper')">
        <div class="flex items-center gap-2 mb-2">
          ...
          <USwitch v-model="configMeta.badge" checked-icon="i-heroicons-check-20-solid" unchecked-icon="i-heroicons-x-mark-20-solid" />
        </div>
        <div class="flex items-center gap-2">
          <span class="text-sm text-gray-500">{{ $t('config.icon.group') }}</span>
          <USwitch v-model="configMeta.group" checked-icon="i-heroicons-check-20-solid" unchecked-icon="i-heroicons-x-mark-20-solid" />
        </div>
      </FormGroup>
